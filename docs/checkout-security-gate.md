...
The signed price is the trusted server-calculated PRE-DISCOUNT configured unit price used as the Draft Order priceOverride, in checkout/presentment currency. It is compared with the completed Order/webhook line_items[].price_set.presentment_money.amount. It is not a post-discount or final-paid price. Currency is order.presentment_currency (falling back to order.currency when absent), and must match the corresponding money-set currency. The existing wallpaper quantity contract is exactly 1, so this is also the configured line price.
...
Shopify discount codes and automatic discounts may apply after the pre-discount price is signed. The existence or amount of total_discount / discount_allocations does not itself cause checkout security FAIL; these fields neither alter the proof nor replace the pre-discount price comparison. A tampered or mismatched pre-discount unit price still causes CHECKOUT_PRICE_MISMATCH, including on discounted orders. Variant, SKU, quantity=1, master, payload/hash, dimensions, instance, currency, HMAC and replay checks remain unchanged. Accessories remain outside the configured-line proof contract.
